...
function SeriesWatchlist() {
  ...
  const ascRatings = () => {
    const ascsortWwatchList = watchList.sort((seriesA, seriesB) => {
      return seriesA.vote_average - seriesB.vote_average;
    });
  ...
  const descRatings = () => {
    const descsortWwatchList = watchList.sort((seriesA, seriesB) => {
      return seriesB.vote_average - seriesA.vote_average;
    });
        ...
        <table className="w-full border-collapse bg-white text-left text-sm text-black">
          ...
          <tbody className="divide-y divide-gray-100 border-t border-black">
            {watchList
              ...
              .map((serie) => (
                <tr className="hover:bg-yellow-500" key={serie.id}>
                  ...
                  <td className="pl-6 py-4 font-bold">{serie.vote_average}</td>
                  <td className="pl-6 py-4 font-bold">
                    {Math.floor(serie.popularity)}
                  </td>
                  <td className="pl-2 py-4">{genre(serie.genre_ids[0])}</td>
                </tr>
              ))}
          </tbody>
        </table>
